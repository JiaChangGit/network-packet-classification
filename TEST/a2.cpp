// g++ -O3 -o a2 a2.cpp -DDEBUG -DCOVER_MANY -gstabs
// ./a2
...
void convertToElementaryIntervals(std::vector<Rule5D>& rules,
  ...
  for (size_t id = 0; id < rule5V_num; ++id) {
    ...
    elements_id_tmp.reserve(rule5V_num);
    for (size_t axis = 0; axis < MAXDIMENSIONS; ++axis) {
      ...
      for (size_t i = 0; i < boundSize - 1; ++i) {
        ...
        if (low >= rules[id].range[axis][LowDim] &&
          ...
          elements_id_tmp.emplace_back(i);  // 存儲基本區間 ID
        }
      }

      rules[id].range[axis][0] = elements_id_tmp.front();
      rules[id].range[axis][1] = elements_id_tmp.back();
...
int main() {
#ifdef COVER_MANY
  const char* CheckCovered_many_path = "CheckCovered_many_a2.txt";
#endif

  // Example usage with 5D space
  // 自定義rule
  // {{MAXDIMENSIONS}, 1, {{14, 14}, {2, 6}, {11, 15}, {7, 18}, {5, 10}}}
  // ==> 五維, priority, {{SIP_0, SIP_1}, {DIP_0, DIP_1}, {SP_0, SP_1}, {DP_0,
  // DP_1}, {PROTC_0, PROTC_1}}
