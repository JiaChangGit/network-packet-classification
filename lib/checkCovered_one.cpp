...
void CoverChecker_one::checkCoverageAndRecord(const std::vector<Rule5D>& rules,
  ...
  for (size_t i = 1; i < rules.size(); ++i) {
    ...
    for (size_t j = 0; j < i; ++j) {
      if (isRuleCovered(rules[i], rules[j])) {
        coveringPriorities.emplace_back(rules[j].priority);
      }
    }
...
inline bool CoverChecker_one::isCovered(const std::array<uint32_t, 2>& range1,
                                        const std::array<uint32_t, 2>& range2) {
  return range1[0] >= range2[0] && range1[1] <= range2[1];
}

inline bool CoverChecker_one::isRuleCovered(const Rule5D& rule,
                                            const Rule5D& other) {
