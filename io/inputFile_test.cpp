...
void InputFile5D_test::loadRule5D_ip_test(const std::vector<Rule5D> &rule5V, const char *fileName)
{
        std::ofstream outFile(fileName);
        if (!outFile.is_open())
        {
...
void InputFile5D_test::loadPacket5D_ip_test(const std::vector<Packet5D> &packet5V, const char *fileName)
{
        std::ofstream outFile(fileName);
        if (!outFile.is_open())
        {
