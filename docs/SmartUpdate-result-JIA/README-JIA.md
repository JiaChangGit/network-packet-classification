ubuntu22.04

---
change following code:

if (fscanf(rule_fp, CB_RULE_FMT,
            &src_ip_0, &src_ip_1, &src_ip_2, &src_ip_3, &src_ip_mask,
            &dst_ip_0, &dst_ip_1, &dst_ip_2, &dst_ip_3, &dst_ip_mask,
            &src_port_begin, &src_port_end, &dst_port_begin, &dst_port_end,
            &proto, &proto_mask, &rule_id) == EOF) {
            fprintf(stderr, "Illegal rule format\n");
            exit(-1);
        }
        
if (fscanf(rule_fp, PRFX_RULE_FMT, &src_ip_0, &src_ip_1, &src_ip_2,
               &src_ip_3, &src_ip_mask, &dst_ip_0, &dst_ip_1, &dst_ip_2,
               &dst_ip_3, &dst_ip_mask, &src_port, &src_port_mask, &dst_port,
               &dst_port_mask, &proto, &proto_mask, &rule_id) == EOF) {
      fprintf(stderr, "Illegal rule format\n");
      exit(-1);
    }
    
if (fscanf(trace_fp, PKT_FMT, &t->pkts[i].val[DIM_SIP].u32,
               &t->pkts[i].val[DIM_DIP].u32, &t->pkts[i].val[DIM_SPORT].u32,
               &t->pkts[i].val[DIM_DPORT].u32, &t->pkts[i].val[DIM_PROTO].u32,
               &t->pkts[i].match) == EOF) {
      fprintf(stderr, "Illegal packet format\n");
      exit(-1);
    }    
        
#define CB_RULE_FMT "@%d.%d.%d.%d/%d\t%d.%d.%d.%d/%d\t%d : %d\t%d : %d\t%x/%x%*x/%*x\n"
#define PKT_FMT "%u\t%u\t%u\t%u\t%u\t%d\t%*d\t%*d\n"
---

---
Valid options:
  -h, --help         display this help and exit
  -r, --rule FILE    specify a rule file for building
  -t, --trace FILE   specify a trace file for searching
  -u, --update FILE  specify a update rule file for searching
  -a, --algorithm ID specify an algorithm, 0:HyperSplit, 1:TSS
  -e  --estimate     specify mode of the estimator, 0:Sleep, 1:Enable
  -s  --system       specify mode of the system, 0:build verifier, 1:build estimator, 2:update verifier, 3:update estimator


# how to run c codes
# HyperSplit
./build/SmartUpdate -a 0 -e 1 -r test/rules/fw1_10K -t test/traces/fw1_10K_trace
# TSS
./build/SmartUpdate -a 1 -e 1 -r test/p_rules/fw1_10K -t test/traces/fw1_10K_trace

# how to run python codes
python python some_script.py -h
---

mkdir build
cd build
cmake ..
make 
ulimit -s 81920

./SmartUpdate -a 0 -e 1 -r ../../classbench_set/ipv4-ruleset/acl1_100k -t ../../classbench_set/ipv4-trace/acl1_100k_trace
